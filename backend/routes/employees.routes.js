...
router.get("/me", auth, (req, res) => {
  ...
  const employee = req.user.employee_id
    ? db.prepare("SELECT * FROM employees WHERE id = ?").get(req.user.employee_id)
    : db.prepare("SELECT * FROM employees ORDER BY id LIMIT 1").get();
  db.close();
...
router.get("/", auth, roles("rh", "admin", "manager"), (req, res) => {
  const db = getDb();
  const employees = db.prepare("SELECT * FROM employees ORDER BY name").all().map(mapEmployee);
  db.close();
  res.json({ employees });
});
