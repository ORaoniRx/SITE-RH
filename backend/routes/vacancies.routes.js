...
router.get("/public", (req, res) => {
  const db = getDb();
  const vacancies = db.prepare(`${vacancyQuery} WHERE v.status = 'Aberta' GROUP BY v.id ORDER BY v.created_at DESC`).all().map(mapVacancy);
  db.close();
  res.json({ vacancies });
});
...
router.get("/", auth, roles("rh", "admin", "manager"), (req, res) => {
  const db = getDb();
  const vacancies = db.prepare(`${vacancyQuery} GROUP BY v.id ORDER BY v.created_at DESC`).all().map(mapVacancy);
  db.close();
  res.json({ vacancies });
});

router.post("/", auth, roles("rh", "admin"), (req, res, next) => {
  try {
    ...
    const vacancy = db.prepare(`${vacancyQuery} WHERE v.id = ? GROUP BY v.id`).get(result.lastInsertRowid);
    db.close();
